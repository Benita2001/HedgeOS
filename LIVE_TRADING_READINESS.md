...
**Real blocker found, not assumed**: the real preflight run in §checkpoint-10 (`scripts/live-preflight.ts`, against the account owner's actual, currently-installed credential) returned `enableInternalTransfer: false`. This means **even a fully correct implementation cannot move a single dollar with today's key** — automatic funding requires either updating that key's permissions or creating a new one with `enableInternalTransfer` granted (a distinct, narrow scope — does not touch withdrawals, which stay disabled). This is the exact next human action for automatic funding specifically, separate from everything else already listed in §6.
