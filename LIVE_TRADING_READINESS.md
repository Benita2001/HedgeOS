...
**Update: permission granted and reconfirmed by a real read-only call.** The blocker described here when first written (`enableInternalTransfer: false`) is resolved — the account owner enabled it, and a fresh `scripts/live-preflight.ts` run against the real credential now returns `enableInternalTransfer: true`, with `enableWithdrawals` still `false` and `ipRestrict` still `true` (unchanged, confirmed in the same call). **The remaining blocker is funding, not permissions** — the Futures wallet showed $0.00 in that same check. Also fixed this session: the shared-wallet reservation gap flagged when this section was written (`reservedFuturesUsd` was a caller-supplied placeholder) is now derived for real from durable DB state — see `docs/FUNDING_READINESS.md`.
