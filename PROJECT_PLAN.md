...
- VERIFIED 2026-09-08 (build-review session): Binance Agent OS MCP server connected and real read-only tool calls made — `spot_tickerPrice`, `spot_exchangeInfo`, `futures_usds_symbolPriceTicker`, `futures_usds_exchangeInformation` (see `docs/INTEGRATION_SURFACES.md` §1, `PROGRESS_LOG.md` checkpoint 8). Its role is operator-side evidence only, formalized in `docs/AGENT_OS_OPERATOR_WORKFLOW.md` — it does not feed the sizing engine. STILL NOT verified: authenticated trading behavior via Agent OS, since that needs Futures-enabled credentials this agent will not create unilaterally.
...
Stock leg: bStock Spot. Hedge leg: TRADIFI_PERPETUAL. Both legs' Agent OS MCP surface is read-only/market-data-and-Spot-schema only — no Futures order/leverage/margin tools exist there, and Agent OS itself has no headless auth, so neither leg's *live execution* could ever run through MCP regardless of credentials. Decision (unchanged): both legs' live path is direct, authenticated REST (`/api/v3/order`, `/fapi/v1/order`, etc.) — now actually implemented (`src/binance/liveExecution.ts`, checkpoint 9), gated behind `assertLiveTradingGate()`, and still blocked on the same human-only steps (a Futures-enabled API key, signing `POST /fapi/v1/stock/contract`, unverified bStock/ADGM-specific eligibility) documented in `LIVE_TRADING_READINESS.md`. The hedge stays in clearly-labeled paper mode for the hackathon submission — live execution is architecturally ready, not account-ready.
